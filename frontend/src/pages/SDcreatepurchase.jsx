...
const SDcreatepurchase = () => {
  ...
  const validateForm = () => {
    ...
    else if (!/^[PR](?:[1-8]0[1-6]|0[1-6])$/.test(formData.apartmentNo)) newErrors.apartmentNo = 'Please enter a valid email';
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (formData.room_type === 'rent') {
      ...
      payload.lease_end_date = formData.lease_end_date || calculateEndDate(); // Ensure this is sent
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-white border border-teal-300 rounded-lg shadow-md">
                    ...
                    {errors.apartmentNo && (
                      <label className="label">
                        <span className="label-text-alt text-error">{errors.apartmentNo}</span>
                      </label>
                    )}
                  </div>
                ...
                <div className="card-actions justify-end gap-4 mt-6 p-4">
                  <button
                    type="submit"
                    className="btn btn-primary text-white bg-teal-600 hover:bg-teal-700 disabled:bg-teal-400 transition-all px-6 py-3 rounded-full flex items-center w-48" // Fixed width
                    disabled={loading}
                  >
                    {loading ? (
                      <>
